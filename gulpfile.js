...
gulp.task('copy:html', () => gulp.src('./app/template/index.html').pipe(gulp.dest(OUT_DIR)));

// copy css to build folder
gulp.task('copy:css', () => gulp.src('./app/template/style.css').pipe(gulp.dest(OUT_DIR)));
...
// Default gulp task, runs all previously defined tasks.
